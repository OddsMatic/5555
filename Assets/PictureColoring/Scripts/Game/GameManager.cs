...
namespace BizzyBeeGames.PictureColoring
{
	public class GameManager : SingletonComponent<GameManager>, ISaveable
	{
		...
		{
			JSONNode saveJson = SaveManager.Instance.LoadSave(this);
			...
			JSONArray levelSaveDatasJson = saveJson["levels"].AsArray;
			...
			LoadHastSetValues(saveJson["awarded"].Value, awardedLevels);
			LoadHastSetValues(saveJson["unlocked"].Value, unlockedLevels);
